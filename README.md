...
| 09:20 生成“前收盘 + 隔夜新闻”作战计划，可选 AI 压缩说明 | AI 凭空报价或自动下单 |
...
`notify-daily` 会先把日K截断到最近完整交易日，再为真实股票与 ETF 持仓计算 MA/ATR/RSI/MACD/BOLL/KDJ、成交量、近期高低点、成本和仓位；确定性规则给出持有/观望/减仓/小幅加仓、买卖参考区间、失效位、目标位和仓位变化。Telegram 使用紧凑行式摘要，展示风险重点最多 3 只及仓位重点最多 2 只；新闻、全量指标、纪律和全部持仓在随消息发送的 `my-portfolio-detail.md` 附件中。行情/新闻降级或关键指标缺失会降低综合置信度；建议与纪律冲突时提示人工复核。
